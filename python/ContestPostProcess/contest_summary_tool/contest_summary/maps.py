...
def render_map(df, title, outdir, args):
    if args.map == "countries":
        render_countries_map(df, title, outdir, include_lower48=args.include_lower48)
...
def render_countries_map(df, title, outdir, include_lower48=False):
...
def render_na_states_dx_map(df, title, outdir):
    ...
    na_world = world[world["name"].isin({"United States of America", "Canada", "Mexico", "Greenland"})].copy()
